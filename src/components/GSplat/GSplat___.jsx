...
function Focus() {
  ...
  useFrame(({ controls, camera }) => {
    if (controls) {
      camera.getWorldPosition(tv)
      flatCursor.current.position.lerpVectors(controls.target, camera.position, 0.3)
      flatCursor.current.lookAt(tv)

      ball.current.position.copy(controls.target)
    }
  })
  return (
    <>
      ...
      <Box
        ...
        onClick={(ev) => {
          ...
          //   diff.copy(ev.point)
          //   diff.sub(controls.object.position)
          //   //
          //   anime({
          //     targets: [controls.object.position],
          //     x: controls.object.position.x + diff.x,
          //     y: controls.object.position.y + diff.y,
          //     z: controls.object.position.z + diff.z,
          //     duration: 1000,
          //     easing: 'easeOutQuad',
          //   })
          // }
      ...
      <Box
        ...
        onPointerMove={(ev) => {
          //
          // o3.position.copy(ev.point)

          if (light.current) {
            //
            light.current.position.copy(ev.point)
            // light.current.position.addScaledVector(ev.face.normal, -20.0)
            light.current.needsUpdate = true
          }
        }}>
        <meshBasicMaterial color={'#0000ff'} wireframe={true} side={THREE.DoubleSide}></meshBasicMaterial>
      </Box>
  ...
  loadData(src, camera, object, controls) {
    ...
    fetch(src)
      ...
      .then((buffer) => {
        ...
        let pl = new THREE.PlaneGeometry(2 * 2.5, 2 * 2.5)
        ...
        stdMat.onBeforeCompile = (shader) => {
          ...
          void main() {
            ...
            // transformed = vec3(rotationY((dissolveFactor) * 3.141592 * 2.0) * vec4(transformed, 1.0));
          ...
          void main() {
            ...
            vec3 totalEmissiveRadiance = emissive;// + v_Color.rgb;
