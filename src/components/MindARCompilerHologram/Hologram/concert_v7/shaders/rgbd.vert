...
void main() {
    ...
    vec2 basetex = offset * texSize.xy + floor(position.xy * textureStep * texSize.zw) * texSize.xy;

    vec2 imageCoordinates = crop.xy + (basetex * crop.zw);
    basetex.y = 1.0 - basetex.y;
    basetex.x = 1.0 - basetex.x;

    vec2 depthTexCoord = (basetex) * (vec2(1.0, 0.5)) + centerpix;
    vec2 colorTexCoord = (basetex) * (vec2(1.0, 0.5)) + vec2(0.0, 0.5) + centerpix;

    depthTexCoord.x *= scale.x;
    depthTexCoord.y *= scale.y;
    colorTexCoord.x *= scale.x;
    colorTexCoord.y *= scale.y;

    depthTexCoord.x += offset.x;
    depthTexCoord.y += offset.y;
    colorTexCoord.x += offset.x;
    colorTexCoord.y += offset.y;
    ...
    vec4 worldPos = extrinsics * vec4((imageCoordinates * imageDimensions - principalPoint) * (z) / focalLength, (z + merger), 1.0);
    worldPos.w = 1.0;


    vec4 wwpos = modelViewMatrix * worldPos;

    gl_Position = projectionMatrix * wwpos;

    if (isPoints) {
      float dist = length(cameraPosition - worldPos.rgb);

      float pointSize = 0.5 + pow(dist, 1.0 - dist / 50.0);
